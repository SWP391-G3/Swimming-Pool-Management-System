...
CREATE TABLE Pool_Mark (
    mark_id INT IDENTITY(1,1) PRIMARY KEY,
    user_id INT NOT NULL,
    pool_id INT NOT NULL,
    marked_at DATETIME NOT NULL DEFAULT GETDATE(),
    CONSTRAINT FK_PoolMark_User FOREIGN KEY (user_id) REFERENCES Users(user_id),
    CONSTRAINT FK_PoolMark_Pool FOREIGN KEY (pool_id) REFERENCES Pools(pool_id),
    CONSTRAINT UQ_PoolMark UNIQUE(user_id, pool_id) -- mỗi người dùng chỉ được đánh dấu 1 lần với 1 bể
);
...
CREATE TABLE Refunds (
    ...
    refunded_by INT, -- user_id của nhân viên xử lý
    CONSTRAINT FK_Refunds_Ticket FOREIGN KEY (ticket_id) REFERENCES Ticket(ticket_id),
    CONSTRAINT FK_Refunds_User FOREIGN KEY (refunded_by) REFERENCES Users(user_id)
);
