...
public class PoolDAO extends DBContext {
    ...
    public static void main(String[] args) {
        ...
        List<Pool> pools = dao.getPoolsByBranchId(1);
        for (Pool pool : pools) {
    ...
    public boolean canDelete(int pool_id) {
        String query = "SELECT pool_status FROM Pools WHERE pool_id = ?";
        try (PreparedStatement stmt = connection.prepareStatement(query)) {
            stmt.setInt(1, pool_id);
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    return !rs.getBoolean("pool_status"); // Nếu pool_status = false (0) thì có thể xóa
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return false; // Mặc định không cho xóa nếu có lỗi hoặc không tìm thấy
    }

    public boolean isPoolNameExist(String pool_name) {
        String sql = "SELECT 1 FROM Pools WHERE pool_name = ?";
        try (PreparedStatement st = connection.prepareStatement(sql)) {
    ...
    public boolean checkBookingForDelete(int pool_id) {
        String sql = """
