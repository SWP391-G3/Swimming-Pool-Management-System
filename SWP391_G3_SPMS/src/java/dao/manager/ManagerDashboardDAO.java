...
public class ManagerDashboardDAO extends DBContext {
    ...
    public List<RevenueChart> getPeakHourAnalysis(int branchId) throws SQLException {
        String sql = """
            SELECT 
                CASE 
                    WHEN DATEPART(hour, b.start_time) BETWEEN 6 AND 9 THEN N'Sáng sớm (6-9h)'
                    WHEN DATEPART(hour, b.start_time) BETWEEN 9 AND 12 THEN N'Buổi sáng (9-12h)'
                    WHEN DATEPART(hour, b.start_time) BETWEEN 12 AND 15 THEN N'Buổi trưa (12-15h)'
                    WHEN DATEPART(hour, b.start_time) BETWEEN 15 AND 18 THEN N'Chiều (15-18h)'
                    WHEN DATEPART(hour, b.start_time) BETWEEN 18 AND 21 THEN N'Tối (18-21h)'
                    ELSE N'Ngoài giờ'
                END as label,
                COUNT(*) as value,
