 ...
 * @author Lenovo
 */
@WebServlet(name = "UpdatePoolServlet", urlPatterns = {"/updatePoolAdmin"})
public class GetPoolUpdateServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet UpdatePoolServlet</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet UpdatePoolServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (pool != null) {
                ...
                request.getRequestDispatcher("updatePool.jsp").forward(request, response);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            pool = new Pool(pool_id, pool_name, pool_road, pool_address, max_slot, open_time, close_time, pool_status, null, updateDate);
            dao.updatePool(pool);
