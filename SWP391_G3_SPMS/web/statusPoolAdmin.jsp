...
<html>
    <head>
        <title>Kết quả lọc theo trạng thái</title>
            ...
            <div class="row">
                ...
                <main class="col-md-10 ms-sm-auto col-lg-10 main-content">
                    <div class="header-bar">
                        <h2 class="fw-bold text-primary">Kết quả lọc theo trạng thái</h2>
                        <a href="#"><i class="bi bi-person-circle"></i></a>
                    </div>
                    <% 
                        String key_name = String.valueOf(request.getAttribute("key_status"));
                    %>
                    <p class="text-muted">Kết quả cho từ khóa: <strong>"<%= key_name != null && key_name.equals("true") ? "Đang hoạt động" : "Hủy hoạt động" %>"</strong></p>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover align-middle">
                            ...
                            <% 
                                ...
                                String nowStatus = String.valueOf(request.getAttribute("nowstatus"));
                    ...
                    <div class="pagination">
                        <% if (currentPage > 1) { %>
                        <a class="btn btn-primary" href="statusPoolAdmin?page=<%= currentPage - 1 %>&nowStatus=<%= nowStatus %>">Previous</a>
                        <% } %>

                        <% for (int i = startPage; i <= endPage; i++) { %>
                        <a class="btn <%= (i == currentPage ? "btn-success" : "btn-secondary") %>" href="statusPoolAdmin?page=<%= i %>&nowStatus=<%= nowStatus %>"><%= i %></a>
                        <% } %>

                        <% if (currentPage < totalPages) { %>
                        <a class="btn btn-primary" href="statusPoolAdmin?page=<%= currentPage + 1 %>&nowStatus=<%= nowStatus %>">Next</a>
                        <% } %>
                    </div>
                </main>
            </div>
