...
//        }
    ...
    sql.append("SELECT dr.report_id, dr.staff_id, dr.pool_id, dr.branch_id, ")
        ...
        .append("u.full_name as staff_name, p.pool_name, b.branch_name, pd.device_name as original_device_name ")
        ...
        .append("LEFT JOIN Pool_Device pd ON dr.device_id = pd.device_id ")
    ...
    if (keyword != null && !keyword.trim().isEmpty()) {
        sql.append("AND (dr.device_name LIKE ? OR dr.report_reason LIKE ? OR u.full_name LIKE ? OR pd.device_name LIKE ?) ");
        String searchKeyword = "%" + keyword.trim() + "%";
        params.add(searchKeyword); // dr.device_name
        params.add(searchKeyword); // dr.report_reason
        params.add(searchKeyword); // u.full_name
        params.add(searchKeyword); // pd.device_name
    }
    ...
    try (PreparedStatement ps = connection.prepareStatement(sql.toString())) {
        ...
        while (rs.next()) {
            ...
            // Nếu muốn hiển thị tên thiết bị gốc: rs.getString("original_device_name")
