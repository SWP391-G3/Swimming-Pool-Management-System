...
<html lang="vi">
    <head>
        <meta charset="UTF-8">
        <title>Báo cáo dịch vụ của tôi</title>
    ...
    <body>
        <div class="layout">
            ...
            <div class="content-panel">
                <div class="header">
                    <h2>Báo cáo dịch vụ của tôi</h2>
                    <a href="staffPoolService" class="btn-back">&larr; Quay lại danh sách dịch vụ</a>
                </div>
                <!-- Bộ lọc -->
                <form class="filter-form" method="get" action="staffServiceReport">
                    <select name="status">
                    ...
                    <a href="staffServiceReport" class="btn-reset">Reset</a>
                </form>
                <c:choose>
                    <c:when test="${empty reportList}">
                        <div class="no-service">Chưa có báo cáo dịch vụ nào.</div>
                    </c:when>
                    <c:otherwise>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <button type="button"
                                                    ...
                                                    data-service-name="${fn:escapeXml(r.serviceName)}"
                                                    data-pool-name="${fn:escapeXml(r.poolName)}"
                                                    data-report-reason="${fn:escapeXml(r.reportReason)}"
                                                    data-suggestion="${fn:escapeXml(r.suggestion)}"
                                                    data-report-date="${r.reportDate}"
                                                    data-status="${r.status}"
                                                    data-service-id="${r.serviceId}"
                                                    data-report-id="${r.reportId}"
                                                    data-manager-note="${fn:escapeXml(r.managerNote)}"
                ...
                <div class="pagination">
                    <c:if test="${endPage > 1}">
                        <c:if test="${page > 1}">
                            <a href="staffServiceReport?page=1&name=${fn:escapeXml(name)}&status=${status}&pageSize=${pageSize}">&laquo;</a>
                        </c:if>
                        <c:forEach var="i" begin="1" end="${endPage}">
                            <a href="staffServiceReport?page=${i}&name=${fn:escapeXml(name)}&status=${status}&pageSize=${pageSize}" class="${i == page ? 'active' : ''}">${i}</a>
                        </c:forEach>
                        <c:if test="${page < endPage}">
                            <a href="staffServiceReport?page=${endPage}&name=${fn:escapeXml(name)}&status=${status}&pageSize=${pageSize}">&raquo;</a>
                        </c:if>
                    </c:if>
                </div>
            </div>
        ...
        <!-- Modal Chi tiết -->
        <div id="serviceReportDetailModal" class="custom-modal">
            <div class="custom-modal-content">
                <button class="custom-modal-close" onclick="closeServiceReportDetail()">&times;</button>
                ...
                <div class="custom-modal-body">
                    ...
                    <div><b>Ghi chú quản lý:</b> <span id="modalManagerNote"></span></div>
                </div>
            </div>
        </div>
        <script>
            function openModalFromButton(btn) {
                ...
                const reportReason = btn.dataset.reportReason || '';
                const suggestion = btn.dataset.suggestion || '';
                const reportDate = btn.dataset.reportDate || '';
                const status = btn.dataset.status || '';
                const managerNote = btn.dataset.managerNote || '';
                ...
                document.getElementById('modalManagerNote').innerText = managerNote;

                document.getElementById('serviceReportDetailModal').style.display = 'flex';
            }

            function closeServiceReportDetail() {
                document.getElementById('serviceReportDetailModal').style.display = 'none';
            }
        </script>
