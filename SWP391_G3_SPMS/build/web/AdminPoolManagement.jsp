...
<html>
    ...
    <body class="bg-gray-100 text-gray-800 overflow-x-hidden">
        ...
        <!-- popup báo lỗi không xóa được khi đang có lượt đặt -->
        <div id="bookingWarningPopup" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" style="display:none;">
            <div class="bg-white rounded-lg p-6 shadow-lg max-w-md w-full">
                <h2 class="text-xl font-semibold text-red-600 mb-4 text-center">Không thể xóa bể bơi</h2>
                <p id="popupMessage" class="text-gray-700 mb-4 text-center"></p>
                <div class="text-right">
                    <button onclick="closeWarningPopup()" class="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700">Đóng</button>
                </div>
            </div>
        </div>
        ...
        <%
            ...
            if (errorMessage != null) {
                 errorMessage = errorMessage.replace("\"", "\\\""); // escape dấu " để tránh lỗi JS
        %>
        <script>
            window.onload = function () {
                showWarningPopup("<%= errorMessage %>");
            };
        </script>
        <%
                session.removeAttribute("deleteError"); // Xóa sau khi dùng
            }
        %>

        <!-- Popup thông báo thành công -->
        <div id="successPopup" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" style="display:none;">
            <div class="bg-white rounded-2xl p-8 shadow-2xl max-w-md w-full border border-green-300 animate-fade-in">
                <div class="flex flex-col items-center text-center">
                    <div class="text-green-600 text-4xl mb-2">
                        <i class="fas fa-check-circle"></i> <!-- Font Awesome icon -->
                    </div>
                    <h2 class="text-2xl font-bold text-green-700 mb-2">Thành công</h2>
                    <p id="successPopupMessage" class="text-gray-700 mb-6 text-base leading-relaxed"></p>
                    <button onclick="closeSuccessPopup()" class="bg-green-600 hover:bg-green-700 text-white font-medium px-6 py-2 rounded-full shadow transition-all duration-200">
                        Đóng
                    </button>
                </div>
            </div>
        ...
        <script>
            ...
            function showWarningPopup(message) {
                ...
                document.getElementById("bookingWarningPopup").style.display = "flex";
            }

            function closeWarningPopup() {
                document.getElementById("bookingWarningPopup").style.display = "none";
            }
